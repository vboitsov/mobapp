package ua.org.rshu.fmi.mobapp.view.util.markdownconverter.impl;
...
import ua.org.rshu.fmi.mobapp.view.util.consts.MarkdownParserConst;

import static ua.org.rshu.fmi.mobapp.view.util.consts.WebViewStyleConst.DOC_STYLE;
import static ua.org.rshu.fmi.mobapp.view.util.consts.WebViewStyleConst.HIGHLIGHT_JS_SCRIPT;
...
public class MarkdownConverterImpl implements MarkdownConverter {
    private static final String TAG = "MarkdownConverterImpl";
    ...
    private void makeTablesResponsive(Document doc) {
        for (Element element : doc.getElementsByTag(MarkdownParserConst.TABLE_TAG)) {
            element.wrap(MarkdownParserConst.TABLE_WRAP_DIV_TAG);
        }
    }
    ...
    private void replaceInHeaders(Document doc) {
        Arrays.asList(MarkdownParserConst.TAG_H1, MarkdownParserConst.TAG_H2, MarkdownParserConst.TAG_H3, MarkdownParserConst.TAG_H4, MarkdownParserConst.TAG_H5, MarkdownParserConst.TAG_H6).forEach(tag -> {
            for (Element element : doc.getElementsByTag(tag)) {
                ...
                elementText = elementText.replaceAll(MarkdownParserConst.HASH_TAG_REGEX, MarkdownParserConst.HASH_TAG_REPLACEMENT);
    ...
    private void replaceInParagraphs(Document doc) {
        for (Element element : doc.getElementsByTag(MarkdownParserConst.TAG_P)) {
            String elementText = element.toString();
            elementText = elementText.substring(3, elementText.length() - 4);
            elementText = replaceTasks(elementText);
            elementText = elementText.replaceAll(MarkdownParserConst.HASH_TAG_REGEX, MarkdownParserConst.HASH_TAG_REPLACEMENT);
            elementText = elementText.replaceAll(MarkdownParserConst.NEW_LINE_REGEX, MarkdownParserConst.NEW_LINE_REPLACEMENT);
            element.text(elementText);
        }
    }
    ...
    private void replaceInHyperLinks(Document doc) {
        for (Element element : doc.getElementsByTag(MarkdownParserConst.TAG_A)) {
            String elementText = element.toString();
            elementText = elementText.substring(3, elementText.length() - 4);
            elementText = replaceTasks(elementText);
            elementText = elementText.replaceAll(MarkdownParserConst.NEW_LINE_REGEX, MarkdownParserConst.NEW_LINE_REPLACEMENT);
    ...
    private void replaceInListItems(Document doc) {
        for (Element element : doc.getElementsByTag(MarkdownParserConst.TAG_LI)) {
            String elementText = element.toString();
            elementText = elementText.substring(4, elementText.length() - 5);
            elementText = replaceTasks(elementText);
            elementText = elementText.replaceAll(MarkdownParserConst.HASH_TAG_REGEX, MarkdownParserConst.HASH_TAG_REPLACEMENT);
            elementText = elementText.replaceAll(MarkdownParserConst.NEW_LINE_IN_LI_REGEX, MarkdownParserConst.NEW_LINE_REPLACEMENT);
            element.text(elementText);
        }
    }
    ...
    private String replaceTasks(String text) {
        Matcher lineWithTaskMatcher = Pattern.compile(MarkdownParserConst.LINE_WITH_TASK_REGEX).matcher(text);
        while (lineWithTaskMatcher.find()) {
            String lineWithTask = lineWithTaskMatcher.group();
            Matcher pureTaskMatcher = Pattern.compile(MarkdownParserConst.TASK_REGEX).matcher(lineWithTask);
    ...
    private String replaceBracketsWithCheckboxTag(String task) {
        ...
        String checkbox = MarkdownParserConst.CHECKBOX_UNCHECKED_TAG;
        if (mark.equals(MarkdownParserConst.TASK_X_SMALL_MARK) || mark.equals(MarkdownParserConst.TASK_X_BIG_MARK)) {
            checkbox = MarkdownParserConst.CHECKBOX_CHECKED_TAG;
        }
    ...
    private String replaceGtAndLt(Document doc) {
       return doc.toString()
               .replaceAll(MarkdownParserConst.LT1_CODE, MarkdownParserConst.LT_SYMBOL)
               .replaceAll(MarkdownParserConst.GT1_CODE, MarkdownParserConst.GT_SYMBOL)
               .replaceAll(MarkdownParserConst.LT2_CODE, MarkdownParserConst.LT_SYMBOL)
               .replaceAll(MarkdownParserConst.GT2_CODE, MarkdownParserConst.GT_SYMBOL)
               .replaceAll(MarkdownParserConst.LT3_CODE, MarkdownParserConst.LT_SYMBOL)
               .replaceAll(MarkdownParserConst.GT3_CODE, MarkdownParserConst.GT_SYMBOL);
    }
